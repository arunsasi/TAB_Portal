            ...
            if(!empty($code))
            {
                ...
                elseif ($verificatioCode === $useCode) 
                {
                    ...
                    if($stat)
                    {
                        ...
                        $subject = "Natana";
                        #$data['email_subject'] = $subject;
                        $url = site_url('user_registration/validate-email?mail='.$email.'token='.$data['email_verification_code']);
                        
                        $content = "Dear ".$data['name'].", <br/>Please click the link to reset your password: <br/><a href='".$url."' target='blank'> Email verification link </a>";
                        #$content = $this->load->view('user/mail_template_new',$data,TRUE);
