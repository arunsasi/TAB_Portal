...
class CompanyApi extends CI_Controller {
                    ...
                    { 
                            ...
                            {   
                                if($data['email'] != NULL)
                                {
                                    ...
                                    $url = site_url('user_registration/validate-email?token='.$data['email_verification_code']);
	...
	{
		$data['email'] = $this->input->get('mail');
		$data['verificatio_code'] = $this->input->get('id');
        $data['id'] = '';       
        if(isset($data['email'])&& trim($data['email']!='') && isset($data['verificatio_code']) && trim($data['verificatio_code']!=''))
        {
            $code = $this->reg_model->get_email($data); 
            if($code->num_rows()>0)
            {
                foreach($code->result() as $codes)
                {
                    $data['user_code'] = $codes->emailVerificationCode;
                    $data['id'] = $codes->id;  
                }
                if (($data['verificatio_code']) === ($data['user_code'])) 
                {
                    $data['stat'] = $this->reg_model->email_verification($data);
                    if($data['stat'])
                    {
