...
class CompanyApi extends CI_Controller {
	...
	{   
            ...
            $this->form_validation->set_rules('email', 'Email', 'required|strip_tags|valid_email|matches[confirm_email]|is_unique[users.email]');
			...
			else
			{
                    ...
                    if($emailcnt > 0)
                    {
                            $result = array('status' => FALSE,'msg'=> 'Registration Failed. Email ID Already exist.');
                    }
                    else
                    { 
                            //user data insertion......
        ...
        if(isset($email)&& trim($email!='') && isset($verificatio_code) && trim($verificatio_code!=''))
        {
            ...
            if(!empty($code))
            {
                ...
                elseif ($verificatio_code === $useCode) 
                {
                    ...
                    $condtion   = ['id' => $userId];
                    $stat = $this->commonModel->updateData('user',$data,$condtion);
                    if($stat)
                    {
